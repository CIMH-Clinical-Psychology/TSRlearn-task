...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- BIDS event logger with trigger logging ---
    # Drop-in replacement for the 'Begin Experiment' code of `bids_logging_functions`.
    #
    # Time base: everything is globalClock, the same clock win.getFutureFlipTime()
    # uses, so tStartRefresh and trigger timestamps are directly comparable.
    # Onsets are expressed relative to t0 (logger creation), so they start near 0.
    ...
    # NOTE: do NOT import STARTED / FINISHED / core here.
    # Builder pastes this block inside run(), so an import would make those names
    # local to run() and break `thisExp.status = STARTED` at the top of the
    # function (UnboundLocalError). They are already available module-level.
    ...
    class BIDSLogger:
        def __init__(self, win, clock, default_cols=None,
            ...
            clock            : globalClock - MUST be the same clock the window uses
            send_fn          : the actual trigger function, e.g. meg_triggers.send_trigger
            ...
            if clock is None:
                raise ValueError("BIDSLogger needs the clock used by the window "
                                 "(globalClock), otherwise onsets and triggers "
                                 "end up on different time bases.")
            self.t0 = self.now()
        ...
        def now(self):
            ...
            tStartRefresh comes from win.getFutureFlipTime(clock=None), which
            returns times on logging.defaultClock - and run() sets that to
            globalClock. Using core.getTime() here instead would be offset by the
            gap between PsychoPy import and globalClock creation (dialog + window
            setup), which can be tens of seconds.
    
            format='float' is required because globalClock is created with an ISO
            strftime format, so a bare getTime() returns a string.
            """
            return float(self.clock.getTime(format='float'))
    
        def _rel(self, t):
            """Convert absolute time to onset relative to experiment start."""
        ...
        def schedule_onset(self, stim, **extra_cols):
            ...
            if getattr(stim, "status", None) == STARTED and stim not in self.active:
                ...
                # a trigger for this stim may have fired before the row existed
        ...
        def mark_offset(self, stim):
            ...
            if getattr(stim, "status", None) == FINISHED and stim in self.active:
                ...
                # stim never finished cleanly (e.g. routine force-ended):
                # drop it from active so the next trial can reopen it
                self.active.pop(stim, None)
    
        def close_trial(self):
            """
            Call in the End Routine of the last routine of each trial (ITI).
    
            Some components never reach FINISHED - e.g. spoken_word, when the
            routine is ended by the feedback logic before the audio has played
            out. Without this, they stay in `active` forever and every later
            schedule_onset() for them is silently skipped.
            """
            t_now = self.now()
            for stim, idx in list(self.active.items()):
                if np.isnan(self.rows[idx].get("duration", np.nan)):
                    # component was cut short: mark duration up to now, and flag it
                    self.rows[idx]["duration"] = self._rel(t_now) - self.rows[idx]["onset"]
                    self.rows[idx]["truncated"] = 1
            self.active.clear()
            # a trigger queued for a stim whose row never appeared would otherwise
            # be back-filled onto the *next* trial's row
            self.pending.clear()
    
        def add_instant(self, trial_type, onset=None, **extra_cols):
            """
            One-shot event with no duration, e.g. a button press.
    
            onset : absolute time on self.clock. Pass this for anything logged
                    later than it happened - a response logged in End Routine
                    should use key_resp.tStartRefresh + key_resp.rt, not the
                    time End Routine happens to run.
            """
        ...
        def _record_trigger(self, value, t_abs, stim=None, label=None, **extra):
            if stim is not None:
                if stim in self.active:
                    ...
                    # row not created yet -> back-fill on the next schedule_onset
        ...
        def trigger_report(self):
            """Quick console summary of stimulus-to-trigger delays, per label."""
            from collections import defaultdict
            by_label = defaultdict(list)
            for r in self.rows:
                if r.get("trigger_delay") is not None and r.get("type_of_stimulus") != "trigger":
                    by_label[r.get("component_label", "?")].append(r["trigger_delay"] * 1000)
            lines = []
            for lab, d in sorted(by_label.items()):
                d = np.asarray(d)
                lines.append(f"{lab:>24s}  n={len(d):4d}  "
                             f"median={np.median(d):6.2f} ms  "
    ...
    conditions_file = f"sequences/localizer_conditions_things_0{p_num}.xlsx"
    ...
    progress_bar_breaks = visual.Progress(
        ...
        progress=0.0,
        pos=(-0.2, -0.07), size=(0.4, 0.015), anchor='center-left', units='height',
    ...
    bids = BIDSLogger(
        ...
        default_cols=dict(subject=expInfo.get("participant"))
    )
    ...
    bids.trigger(trigger_numbers_dict["start/end"], label="exp_start", on_flip=False)
    ...
    "If you press the red button, the fixation cross will\n"
    "briefly change color to show whether your response\n"
    " was correct.\n\n"
    "[Press any key to continue.]"
    )
    ...
    "Wenn Sie die rote Taste gedrückt haben, ändert das Fixationskreuz\n"
    "kurz seine Farbe, um anzuzeigen, ob Ihre Antwort richtig war.\n\n"
    "[Drücken Sie eine beliebige Taste, um fortzufahren.]"
    )
            ...
            if fdback == 1:
                if match_idx==0:  # Mismatch trial - button press required
                    if (not feedback_given):
                        ...
                        # when mismatch, alwasy give feedback
    ...
    for thisTrial in trials:
        ...
        current_trial_number = trials.thisN + 1 + buffer_num # make it start at 1
        # avoid the 4 buffer trials in beginning
        ...
        while continueRoutine:
            ...
            if frameN == 0: 
                ...
                bids.trigger(concept_num, stim=image, label="image")
        ...
        spoken_word.seek(0)
        fix_cross.setColor('white', colorSpace='rgb')
        ...
        while continueRoutine:
            ...
            if frameN == 0: 
                ...
                win.timeOnFlip(spoken_word, 'tStartRefresh')
                bids.trigger(trigNumber, stim=spoken_word, label="word")
            ...
            if spoken_word.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if (key_resp.keys is not None) and (not resp_trig_giv):
                ...
                bids.add_instant("response",
                    component_label="possible_button_press",
                    response=key_resp.keys,
                    response_time=key_resp.rt,
                    press_onset_est=(key_resp.tStartRefresh + key_resp.rt) - bids.t0,
                    correct=key_resp.corr,
                    expected_response=("m" if answer_correct is None else answer_correct),
                    block_num=block_num, trial_num=trial_num,
                    wrd_img_match=int(1 - is_mismatch))
            
                bids.trigger(response_trigger, label="response", on_flip=False,
                             block_num=block_num, trial_num=trial_num)
                resp_trig_giv = True
            ...
            if fdback == 1:
                if is_mismatch==1:  # Mismatch trial - button press required
                    ...
                    if feedback_given and not feedback_trigger_sent:
                       ...
                       bids.trigger(feedback_trigger_code, label="feedback", on_flip=True,
                                     block_num=block_num, trial_num=trial_num)
        ...
        if key_resp.keys is None:
            bids.add_instant("response",
                ...
                block_num=block_num, trial_num=trial_num,
                wrd_img_match=int(1 - is_mismatch))
        # Run 'End Routine' code from feedback_control_trigger
        fix_cross_prc.setAutoDraw(False)
        ...
        while continueRoutine:
            ...
            if frameN==0:
                ...
                bids.trigger(fixcross_trigger, stim=fix_cross_2, label="fix_cross_iti")
        ...
        while continueRoutine:
            ...
            if frameN == 0 and run_break:
                ...
                bids.trigger(breakstart_trigger, stim=breaks_instruction, label="break_start")
        ...
        if run_break and not end_trig_send:
            ...
            bids.trigger(breakend_trigger, label="break_end", on_flip=False)
    ...
    while continueRoutine:
        ...
        if frameN==0:
            ...
            bids.trigger(trigger_numbers_dict["start/end"], stim=instruction_end, label="exp_end")
