...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('continue_button_22') is None:
        # initialise continue_button_22
        continue_button_22 = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='continue_button_22',
        )
    ...
    if deviceManager.getDevice('continue_button_19') is None:
        # initialise continue_button_19
        continue_button_19 = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='continue_button_19',
        )
    ...
    if deviceManager.getDevice('continue_button_20') is None:
        # initialise continue_button_20
        continue_button_20 = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='continue_button_20',
        )
    ...
    if deviceManager.getDevice('continue_button_21') is None:
        # initialise continue_button_21
        continue_button_21 = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='continue_button_21',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    leftside_retr = (-0.05,0)
    rightside_retr = (0.05,0)
    ...
    context_img_block = "stimuli/default.png"
    # Run 'Begin Experiment' code from bids_logging_functions
    # --- BIDS event logger with trigger logging (TSRlearn_main-blocks) ---
    # Drop-in replacement for the 'Begin Experiment' code of `bids_logging_functions`.
    #
    # Time base: globalClock, the same clock win.getFutureFlipTime(clock=None) uses,
    # so tStartRefresh and trigger timestamps are directly comparable.
    # Onsets are relative to t0 (logger creation), so they start near 0.
    #
    # NOTE: do NOT import STARTED / FINISHED / core here. Builder pastes this block
    # inside run(), so an import would make those names local to run() and break
    # `thisExp.status = STARTED` at the top of the function (UnboundLocalError).
    # Path / csv / numpy / atexit are already imported by `import_packages`.
    
    
    class BIDSLogger:
        def __init__(self, win, clock, default_cols=None,
                     send_fn=None, log_trigger_rows=True):
            """
            win              : the PsychoPy window (needed for callOnFlip)
            clock            : globalClock - MUST be the clock the window uses
            send_fn          : meg_triggers.send_trigger
            log_trigger_rows : if True, every trigger also gets its own row
            """
            self.win = win
            self.clock = clock
            self.send_fn = send_fn
            self.log_trigger_rows = log_trigger_rows
    
            self.rows = []
            self.active = {}      # stim -> row index (open, awaiting offset)
            self.pending = {}     # stim -> trigger fired before its row existed
            self.defaults = default_cols or {}
            if clock is None:
                raise ValueError("BIDSLogger needs globalClock, otherwise onsets "
                                 "and triggers end up on different time bases.")
            self.t0 = self.now()
            ...
            self.col_order = [
                ...
                "onset", "duration",
                "type_of_stimulus",
                "trigger_value", "trigger_onset", "trigger_delay",
                "response_time", "press_onset_est", "response",
        ...
        # ------------------------------------------------------------ timing --
        def now(self):
            """Current time on the SAME clock as tStartRefresh."""
            return float(self.clock.getTime(format='float'))
        ...
        def schedule_onset(self, stim, **extra_cols):
            """Call every frame; logs on the first frame the stim is STARTED."""
            if getattr(stim, "status", None) == STARTED and stim not in self.active:
                t_on = getattr(stim, "tStartRefresh", None)
                if t_on is None:
                    t_on = self.now()
    
                row = {"onset": self._rel(t_on), "duration": np.nan}
                row.update(self.defaults)
                row.update(extra_cols)
    
                trig = self.pending.pop(stim, None)
                if trig is not None:
                    self._attach_trigger(row, trig["value"], trig["t"])
    
                self.rows.append(row)
                self.active[stim] = len(self.rows) - 1
    
        def mark_offset(self, stim):
            """Call in End Routine; closes the row and computes duration."""
            if getattr(stim, "status", None) == FINISHED and stim in self.active:
                idx = self.active.pop(stim)
                t_off = getattr(stim, "tStopRefresh", None)
                if t_off is None:
                    t_off = self.now()
                self.rows[idx]["duration"] = self._rel(t_off) - self.rows[idx]["onset"]
            else:
                # never finished cleanly - free it so the next trial can reopen it
                self.active.pop(stim, None)
    
        def close_trial(self):
            """
            Call at the end of each trial. Components that never reach FINISHED
            (routine ended early by a response or animation) would otherwise stay
            in `active` forever and every later schedule_onset() for them would be
            silently skipped.
            """
            t_now = self.now()
            for stim, idx in list(self.active.items()):
                if np.isnan(self.rows[idx].get("duration", np.nan)):
                    self.rows[idx]["duration"] = self._rel(t_now) - self.rows[idx]["onset"]
                    self.rows[idx]["truncated"] = 1
            self.active.clear()
            self.pending.clear()
    
        def add_instant(self, given_name, onset=None, **extra_cols):
            """
            One-shot event with no duration, e.g. a button press.
    
            onset : absolute time on self.clock. Pass this for anything logged
                    later than it happened.
            """
            t = self.now() if onset is None else float(onset)
            row = {"onset": self._rel(t), "duration": 0.0,
        ...
        # -------------------------------------------------------- trigger log --
        def trigger(self, value, stim=None, label=None, on_flip=True, **extra):
            """
            Send a trigger and log when it was sent.
    
            stim     : link the trigger to this stimulus's row, adding
                       trigger_value / trigger_onset / trigger_delay to it
            label    : component_label for the standalone trigger row
            on_flip  : True  -> fire via win.callOnFlip, i.e. immediately after the
                                buffer swap. Use for anything screen-locked.
                       False -> fire right now. Use for responses and for triggers
                                at the very end of a routine.
            """
            value = int(value)
    
            def _fire():
                t = self.now()                     # timestamp before the send
                try:
                    if self.send_fn is not None:
                        self.send_fn(value)
                except Exception as e:
                    # never let a port error swallow the remaining flip callbacks
                    # (clock.reset / clearEvents are registered on the same flip)
                    print(f"[trigger] send failed for {value}: {e}")
                self._record_trigger(value, t, stim=stim, label=label, **extra)
    
            if on_flip and self.win is not None:
                self.win.callOnFlip(_fire)
            else:
                _fire()
    
        def _attach_trigger(self, row, value, t_abs):
            row["trigger_value"] = value
            row["trigger_onset"] = self._rel(t_abs)
            row["trigger_delay"] = row["trigger_onset"] - row["onset"]
    
        def _record_trigger(self, value, t_abs, stim=None, label=None, **extra):
            if stim is not None:
                if stim in self.active:
                    self._attach_trigger(self.rows[self.active[stim]], value, t_abs)
                else:
                    self.pending[stim] = {"value": value, "t": t_abs}
    
            if self.log_trigger_rows:
                row = {"onset": self._rel(t_abs), "duration": 0.0,
                       "type_of_stimulus": "trigger",
                       "component_label": label or "trigger",
                       "trigger_value": value}
                row.update(self.defaults)
                row.update(extra)
                self.rows.append(row)
        ...
        def save(self, filename_base):
            out = Path(filename_base).with_suffix("")
            ...
            # BIDS requires onset-sorted rows; sort a copy so live indices stay valid
            rows_sorted = sorted(self.rows, key=lambda r: r.get("onset", 0.0))
        ...
        def trigger_report(self):
            """Console summary of stimulus-to-trigger delays, per label."""
            from collections import defaultdict
            by_label = defaultdict(list)
            for r in self.rows:
                if (r.get("trigger_delay") is not None
                        and r.get("type_of_stimulus") != "trigger"):
                    by_label[r.get("component_label", "?")].append(
                        r["trigger_delay"] * 1000)
            lines = []
            for lab, d in sorted(by_label.items()):
                d = np.asarray(d)
                lines.append(f"{lab:>24s}  n={len(d):4d}  "
                             f"median={np.median(d):7.3f} ms  "
                             f"IQR=[{np.percentile(d,25):.3f}, {np.percentile(d,75):.3f}]  "
                             f"max={d.max():7.3f}")
            return "\n".join(lines) if lines else "no linked triggers logged"
    ...
    cx, cy = prompt_pos
    r = 0.25        # distance of every top image from the prompt 
    ...
    instructions_part16 = visual.TextStim(win=win, name='instructions_part16',
        text=None,
        font=font_name,
        pos=(0, 0), draggable=False, height=0.025, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    # Run 'Begin Experiment' code from instruction_part16_text
    
    
    continue_button_22 = keyboard.Keyboard(deviceName='continue_button_22')
    
    # --- Initialize components for Routine "practice_choice_display" ---
    polygon = visual.Rect(
        win=win, name='polygon',
        width=(0.16, 0.16)[0], height=(0.16, 0.16)[1],
    ...
    instructions_choose_2 = visual.TextStim(win=win, name='instructions_choose_2',
        text=None,
        font=font_name,
        pos=instruc_pos, draggable=False, height=0.025, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-8.0);
    ...
    instruction_part13 = visual.TextStim(win=win, name='instruction_part13',
        text=None,
        font=font_name,
        pos=(0, 0), draggable=False, height=0.025, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    # Run 'Begin Experiment' code from instruction_part13_text
    
    
    continue_button_19 = keyboard.Keyboard(deviceName='continue_button_19')
    ...
    instruction_part14 = visual.TextStim(win=win, name='instruction_part14',
        text=None,
        font=font_name,
        pos=(0, 0), draggable=False, height=0.025, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    # Run 'Begin Experiment' code from instruction_part14_text
    
    
    continue_button_20 = keyboard.Keyboard(deviceName='continue_button_20')
    ...
    instruction_part15 = visual.TextStim(win=win, name='instruction_part15',
        text=None,
        font=font_name,
        pos=(0, 0), draggable=False, height=0.025, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    # Run 'Begin Experiment' code from instruction_part15_text
    
    
    continue_button_21 = keyboard.Keyboard(deviceName='continue_button_21')
    
    # --- Initialize components for Routine "retrieval_backg_info" ---
    instruction_info = visual.TextStim(win=win, name='instruction_info',
        text=None,
        font=font_name,
        pos=(0.0, 0), draggable=False, height=0.025, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    ...
    info_example_3 = visual.ImageStim(
        ...
        name='info_example_3', 
        image='default.png', mask=None, anchor='center',
        ori=0.0, pos=(0, -0.2), draggable=False, size=(0.7,None),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
    ...
    text_question_order = visual.TextStim(win=win, name='text_question_order',
        text=None,
        font=font_name,
    ...
    opt_5_prc = visual.TextStim(win=win, name='opt_5_prc',
        ...
        languageStyle='LTR',
        depth=-7.0);
    text_question_retrieval = visual.TextStim(win=win, name='text_question_retrieval',
        text=None,
        font=font_name,
        pos=(0, 0.1), draggable=False, height=0.03, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    info_example_2 = visual.ImageStim(
        ...
        name='info_example_2', 
        image='default.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(0.8,None),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-2.0)
    ...
    text_continue = visual.TextStim(win=win, name='text_continue',
        text=None,
        font=font_name,
        pos=(0, -0.15), draggable=False, height=0.025, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-5.0);
    ...
    scene_image_2 = visual.ImageStim(
        win=win,
        name='scene_image_2', 
        image='default.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(1,1),
    ...
    scene_image_3 = visual.ImageStim(
        win=win,
        name='scene_image_3', 
        image='default.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(1,1),
    ...
    scene_image_4 = visual.ImageStim(
        win=win,
        name='scene_image_4', 
        image='default.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(1,1),
    ...
    bids = BIDSLogger(
        win=win,
        clock=globalClock,
        send_fn=send_trigger,
        default_cols=dict(subject=expInfo.get("participant_ID"))
    )
    ...
    if language == "german": 
        instruction_part2.text = (
    "Um sich die Reihenfolge besser zu merken, verbinden Sie die\n"
    ...
    if language == "german": 
        instruction_part3.text = (
    "In jedem Lerndurchgang sehen Sie unten das aktuelle Bild\n"
    ...
    if language == "german": 
        instruction_part4.text = (
    "Verwenden Sie die linke grüne, die rechte blaue\n"
    ...
    if language == "german": 
        instruction_part5.text = (
    "Nach Ihrer Auswahl erhalten Sie Feedback:\n"
    ...
    if language == "german": 
        instruction_part6.text = (
    "Wichtig: Auch bei einer falschen Antwort bewegt\n"
    ...
    # create an object to store info about Routine instructions_replay_break
    instructions_replay_break = data.Routine(
        name='instructions_replay_break',
        components=[instructions_part16, continue_button_22],
    )
    instructions_replay_break.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # Run 'Begin Routine' code from instruction_part16_text
    if language == "english": 
        instructions_part16.text = (
    ...
    if language == "german": 
        instructions_part16.text = (
    "Nach jedem Lerndurchlauf erscheint für kurze Zeit\n"
    "ein Fixationskreuz.\n\n"
    ...
    # create starting attributes for continue_button_22
    continue_button_22.keys = []
    continue_button_22.rt = []
    _continue_button_22_allKeys = []
    # store start times for instructions_replay_break
    instructions_replay_break.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    instructions_replay_break.tStart = globalClock.getTime(format='float')
    instructions_replay_break.status = STARTED
    thisExp.addData('instructions_replay_break.started', instructions_replay_break.tStart)
    instructions_replay_break.maxDuration = None
    # keep track of which components have finished
    instructions_replay_breakComponents = instructions_replay_break.components
    for thisComponent in instructions_replay_break.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "instructions_replay_break" ---
    instructions_replay_break.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # if instructions_part16 is starting this frame...
        if instructions_part16.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            instructions_part16.frameNStart = frameN  # exact frame index
            instructions_part16.tStart = t  # local t and not account for scr refresh
            instructions_part16.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(instructions_part16, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'instructions_part16.started')
            # update status
            instructions_part16.status = STARTED
            instructions_part16.setAutoDraw(True)
        
        # if instructions_part16 is active this frame...
        if instructions_part16.status == STARTED:
            # update params
            pass
        
        # if instructions_part16 is stopping this frame...
        if instructions_part16.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > instructions_part16.tStartRefresh + max_read_dur-frameTolerance:
                # keep track of stop time/frame for later
                instructions_part16.tStop = t  # not accounting for scr refresh
                instructions_part16.tStopRefresh = tThisFlipGlobal  # on global time
                instructions_part16.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'instructions_part16.stopped')
                # update status
                instructions_part16.status = FINISHED
                instructions_part16.setAutoDraw(False)
        
        # *continue_button_22* updates
        waitOnFlip = False
        
        # if continue_button_22 is starting this frame...
        if continue_button_22.status == NOT_STARTED and tThisFlip >= 1-frameTolerance:
            # keep track of start time/frame for later
            continue_button_22.frameNStart = frameN  # exact frame index
            continue_button_22.tStart = t  # local t and not account for scr refresh
            continue_button_22.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(continue_button_22, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'continue_button_22.started')
            # update status
            continue_button_22.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(continue_button_22.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(continue_button_22.clearEvents, eventType='keyboard')  # clear events on next screen flip
        
        # if continue_button_22 is stopping this frame...
        if continue_button_22.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > continue_button_22.tStartRefresh + max_read_dur-frameTolerance:
                # keep track of stop time/frame for later
                continue_button_22.tStop = t  # not accounting for scr refresh
                continue_button_22.tStopRefresh = tThisFlipGlobal  # on global time
                continue_button_22.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'continue_button_22.stopped')
                # update status
                continue_button_22.status = FINISHED
                continue_button_22.status = FINISHED
        if continue_button_22.status == STARTED and not waitOnFlip:
            theseKeys = continue_button_22.getKeys(keyList=[left_key, right_key, center_key, down_key], ignoreKeys=["escape"], waitRelease=False)
            _continue_button_22_allKeys.extend(theseKeys)
            if len(_continue_button_22_allKeys):
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                win=win, 
                timers=[routineTimer, globalClock], 
                currentRoutine=instructions_replay_break,
            )
            # skip the frame we paused on
            continue
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            instructions_replay_break.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in instructions_replay_break.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in instructions_replay_break.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for instructions_replay_break
    instructions_replay_break.tStop = globalClock.getTime(format='float')
    instructions_replay_break.tStopRefresh = tThisFlipGlobal
    thisExp.addData('instructions_replay_break.stopped', instructions_replay_break.tStop)
    # check responses
    if continue_button_22.keys in ['', [], None]:  # No response was made
        continue_button_22.keys = None
    thisExp.addData('continue_button_22.keys',continue_button_22.keys)
    if continue_button_22.keys != None:  # we had a response
        thisExp.addData('continue_button_22.rt', continue_button_22.rt)
        thisExp.addData('continue_button_22.duration', continue_button_22.duration)
    thisExp.nextEntry()
    ...
    # create an object to store info about Routine rest_practice
    rest_practice = data.Routine(
        name='rest_practice',
        components=[fix_cross_2, rest_instruction],
    )
    rest_practice.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # Run 'Begin Routine' code from set_text_rest_instruction
    if language == "english":
        rest_instruction.text = (
        "Short Break. Please wait,\n"
        "this is not a rest break.")
        
    if language == "german":
        rest_instruction.text = (
        "Kurze Pause. Bitte warten Sie.\n"
        "Dies ist keine Ruhepause")
       
    # store start times for rest_practice
    rest_practice.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    rest_practice.tStart = globalClock.getTime(format='float')
    rest_practice.status = STARTED
    thisExp.addData('rest_practice.started', rest_practice.tStart)
    rest_practice.maxDuration = None
    # keep track of which components have finished
    rest_practiceComponents = rest_practice.components
    for thisComponent in rest_practice.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    rest_practice.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        if fix_cross_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if fix_cross_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > fix_cross_2.tStartRefresh + replay_break_dur-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # if rest_instruction is starting this frame...
        if rest_instruction.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'rest_instruction.started')
            # update status
            rest_instruction.status = STARTED
            rest_instruction.setAutoDraw(True)
        
        # if rest_instruction is active this frame...
        if rest_instruction.status == STARTED:
            # update params
            pass
        
        # if rest_instruction is stopping this frame...
        if rest_instruction.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > rest_instruction.tStartRefresh + replay_break_dur-frameTolerance:
                # keep track of stop time/frame for later
                rest_instruction.tStop = t  # not accounting for scr refresh
                rest_instruction.tStopRefresh = tThisFlipGlobal  # on global time
                rest_instruction.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'rest_instruction.stopped')
                # update status
                rest_instruction.status = FINISHED
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            rest_practice.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in rest_practice.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in rest_practice.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for rest_practice
    rest_practice.tStop = globalClock.getTime(format='float')
    rest_practice.tStopRefresh = tThisFlipGlobal
    thisExp.addData('rest_practice.stopped', rest_practice.tStop)
    thisExp.nextEntry()
    ...
    # create an object to store info about Routine instruction_retrievalques1_02
    instruction_retrievalques1_02 = data.Routine(
        name='instruction_retrievalques1_02',
        components=[instruction_part13, continue_button_19],
    )
    instruction_retrievalques1_02.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # Run 'Begin Routine' code from instruction_part13_text
    if language == "english": 
        instruction_part13.text = (
    "For this, use the left (green) and right (blue) button\n."
    ...
    if language == "german": 
        instruction_part13.text = (
    "Wählen Sie „Ja“ oder „Nein“ mit der linken\n"
    ...
    # create starting attributes for continue_button_19
    continue_button_19.keys = []
    continue_button_19.rt = []
    _continue_button_19_allKeys = []
    # store start times for instruction_retrievalques1_02
    instruction_retrievalques1_02.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    instruction_retrievalques1_02.tStart = globalClock.getTime(format='float')
    instruction_retrievalques1_02.status = STARTED
    thisExp.addData('instruction_retrievalques1_02.started', instruction_retrievalques1_02.tStart)
    instruction_retrievalques1_02.maxDuration = None
    # keep track of which components have finished
    instruction_retrievalques1_02Components = instruction_retrievalques1_02.components
    for thisComponent in instruction_retrievalques1_02.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "instruction_retrievalques1_02" ---
    instruction_retrievalques1_02.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # if instruction_part13 is starting this frame...
        if instruction_part13.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            instruction_part13.frameNStart = frameN  # exact frame index
            instruction_part13.tStart = t  # local t and not account for scr refresh
            instruction_part13.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(instruction_part13, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'instruction_part13.started')
            # update status
            instruction_part13.status = STARTED
            instruction_part13.setAutoDraw(True)
        
        # if instruction_part13 is active this frame...
        if instruction_part13.status == STARTED:
            # update params
            pass
        
        # if instruction_part13 is stopping this frame...
        if instruction_part13.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > instruction_part13.tStartRefresh + max_read_dur-frameTolerance:
                # keep track of stop time/frame for later
                instruction_part13.tStop = t  # not accounting for scr refresh
                instruction_part13.tStopRefresh = tThisFlipGlobal  # on global time
                instruction_part13.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'instruction_part13.stopped')
                # update status
                instruction_part13.status = FINISHED
                instruction_part13.setAutoDraw(False)
        
        # *continue_button_19* updates
        waitOnFlip = False
        
        # if continue_button_19 is starting this frame...
        if continue_button_19.status == NOT_STARTED and tThisFlip >= 3-frameTolerance:
            # keep track of start time/frame for later
            continue_button_19.frameNStart = frameN  # exact frame index
            continue_button_19.tStart = t  # local t and not account for scr refresh
            continue_button_19.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(continue_button_19, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'continue_button_19.started')
            # update status
            continue_button_19.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(continue_button_19.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(continue_button_19.clearEvents, eventType='keyboard')  # clear events on next screen flip
        
        # if continue_button_19 is stopping this frame...
        if continue_button_19.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > continue_button_19.tStartRefresh + max_read_dur-frameTolerance:
                # keep track of stop time/frame for later
                continue_button_19.tStop = t  # not accounting for scr refresh
                continue_button_19.tStopRefresh = tThisFlipGlobal  # on global time
                continue_button_19.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'continue_button_19.stopped')
                # update status
                continue_button_19.status = FINISHED
                continue_button_19.status = FINISHED
        if continue_button_19.status == STARTED and not waitOnFlip:
            theseKeys = continue_button_19.getKeys(keyList=[left_key, right_key, center_key, down_key], ignoreKeys=["escape"], waitRelease=False)
            _continue_button_19_allKeys.extend(theseKeys)
            if len(_continue_button_19_allKeys):
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                win=win, 
                timers=[routineTimer, globalClock], 
                currentRoutine=instruction_retrievalques1_02,
            )
            # skip the frame we paused on
            continue
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            instruction_retrievalques1_02.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in instruction_retrievalques1_02.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in instruction_retrievalques1_02.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for instruction_retrievalques1_02
    instruction_retrievalques1_02.tStop = globalClock.getTime(format='float')
    instruction_retrievalques1_02.tStopRefresh = tThisFlipGlobal
    thisExp.addData('instruction_retrievalques1_02.stopped', instruction_retrievalques1_02.tStop)
    # check responses
    if continue_button_19.keys in ['', [], None]:  # No response was made
        continue_button_19.keys = None
    thisExp.addData('continue_button_19.keys',continue_button_19.keys)
    if continue_button_19.keys != None:  # we had a response
        thisExp.addData('continue_button_19.rt', continue_button_19.rt)
        thisExp.addData('continue_button_19.duration', continue_button_19.duration)
    thisExp.nextEntry()
    ...
    # create an object to store info about Routine instruction_retrievalques2_02
    instruction_retrievalques2_02 = data.Routine(
        name='instruction_retrievalques2_02',
        components=[instruction_part14, continue_button_20],
    )
    instruction_retrievalques2_02.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # Run 'Begin Routine' code from instruction_part14_text
    if language == "english": 
        instruction_part14.text = (
    "At distance 2, there is one image in between,\n"
    ...
    if language == "german": 
        instruction_part14.text = (
    "Bei Distanz 2 liegt ein Bild dazwischen,\n"
    ...
    # create starting attributes for continue_button_20
    continue_button_20.keys = []
    continue_button_20.rt = []
    _continue_button_20_allKeys = []
    # store start times for instruction_retrievalques2_02
    instruction_retrievalques2_02.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    instruction_retrievalques2_02.tStart = globalClock.getTime(format='float')
    instruction_retrievalques2_02.status = STARTED
    thisExp.addData('instruction_retrievalques2_02.started', instruction_retrievalques2_02.tStart)
    instruction_retrievalques2_02.maxDuration = None
    # keep track of which components have finished
    instruction_retrievalques2_02Components = instruction_retrievalques2_02.components
    for thisComponent in instruction_retrievalques2_02.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "instruction_retrievalques2_02" ---
    instruction_retrievalques2_02.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # if instruction_part14 is starting this frame...
        if instruction_part14.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            instruction_part14.frameNStart = frameN  # exact frame index
            instruction_part14.tStart = t  # local t and not account for scr refresh
            instruction_part14.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(instruction_part14, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'instruction_part14.started')
            # update status
            instruction_part14.status = STARTED
            instruction_part14.setAutoDraw(True)
        
        # if instruction_part14 is active this frame...
        if instruction_part14.status == STARTED:
            # update params
            pass
        
        # if instruction_part14 is stopping this frame...
        if instruction_part14.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > instruction_part14.tStartRefresh + max_read_dur-frameTolerance:
                # keep track of stop time/frame for later
                instruction_part14.tStop = t  # not accounting for scr refresh
                instruction_part14.tStopRefresh = tThisFlipGlobal  # on global time
                instruction_part14.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'instruction_part14.stopped')
                # update status
                instruction_part14.status = FINISHED
                instruction_part14.setAutoDraw(False)
        
        # *continue_button_20* updates
        waitOnFlip = False
        
        # if continue_button_20 is starting this frame...
        if continue_button_20.status == NOT_STARTED and tThisFlip >= 3-frameTolerance:
            # keep track of start time/frame for later
            continue_button_20.frameNStart = frameN  # exact frame index
            continue_button_20.tStart = t  # local t and not account for scr refresh
            continue_button_20.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(continue_button_20, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'continue_button_20.started')
            # update status
            continue_button_20.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(continue_button_20.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(continue_button_20.clearEvents, eventType='keyboard')  # clear events on next screen flip
        
        # if continue_button_20 is stopping this frame...
        if continue_button_20.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > continue_button_20.tStartRefresh + max_read_dur-frameTolerance:
                # keep track of stop time/frame for later
                continue_button_20.tStop = t  # not accounting for scr refresh
                continue_button_20.tStopRefresh = tThisFlipGlobal  # on global time
                continue_button_20.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'continue_button_20.stopped')
                # update status
                continue_button_20.status = FINISHED
                continue_button_20.status = FINISHED
        if continue_button_20.status == STARTED and not waitOnFlip:
            theseKeys = continue_button_20.getKeys(keyList=[left_key, right_key, center_key, down_key], ignoreKeys=["escape"], waitRelease=False)
            _continue_button_20_allKeys.extend(theseKeys)
            if len(_continue_button_20_allKeys):
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                win=win, 
                timers=[routineTimer, globalClock], 
                currentRoutine=instruction_retrievalques2_02,
            )
            # skip the frame we paused on
            continue
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            instruction_retrievalques2_02.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in instruction_retrievalques2_02.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in instruction_retrievalques2_02.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for instruction_retrievalques2_02
    instruction_retrievalques2_02.tStop = globalClock.getTime(format='float')
    instruction_retrievalques2_02.tStopRefresh = tThisFlipGlobal
    thisExp.addData('instruction_retrievalques2_02.stopped', instruction_retrievalques2_02.tStop)
    # check responses
    if continue_button_20.keys in ['', [], None]:  # No response was made
        continue_button_20.keys = None
    thisExp.addData('continue_button_20.keys',continue_button_20.keys)
    if continue_button_20.keys != None:  # we had a response
        thisExp.addData('continue_button_20.rt', continue_button_20.rt)
        thisExp.addData('continue_button_20.duration', continue_button_20.duration)
    thisExp.nextEntry()
    ...
    # create an object to store info about Routine instruction_refl_period_02
    instruction_refl_period_02 = data.Routine(
        name='instruction_refl_period_02',
        components=[instruction_part15, continue_button_21],
    )
    instruction_refl_period_02.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # Run 'Begin Routine' code from instruction_part15_text
    if language == "english": 
        instruction_part15.text = (
       "During this time, already consider whether the images\n"
    ...
    if language == "german": 
        instruction_part15.text = (
    "Überlegen Sie in dieser Zeit bereits, ob die Bilder\n"
    ...
    # create starting attributes for continue_button_21
    continue_button_21.keys = []
    continue_button_21.rt = []
    _continue_button_21_allKeys = []
    # store start times for instruction_refl_period_02
    instruction_refl_period_02.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    instruction_refl_period_02.tStart = globalClock.getTime(format='float')
    instruction_refl_period_02.status = STARTED
    thisExp.addData('instruction_refl_period_02.started', instruction_refl_period_02.tStart)
    instruction_refl_period_02.maxDuration = None
    # keep track of which components have finished
    instruction_refl_period_02Components = instruction_refl_period_02.components
    for thisComponent in instruction_refl_period_02.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "instruction_refl_period_02" ---
    instruction_refl_period_02.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # if instruction_part15 is starting this frame...
        if instruction_part15.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            instruction_part15.frameNStart = frameN  # exact frame index
            instruction_part15.tStart = t  # local t and not account for scr refresh
            instruction_part15.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(instruction_part15, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'instruction_part15.started')
            # update status
            instruction_part15.status = STARTED
            instruction_part15.setAutoDraw(True)
        
        # if instruction_part15 is active this frame...
        if instruction_part15.status == STARTED:
            # update params
            pass
        
        # if instruction_part15 is stopping this frame...
        if instruction_part15.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > instruction_part15.tStartRefresh + max_read_dur-frameTolerance:
                # keep track of stop time/frame for later
                instruction_part15.tStop = t  # not accounting for scr refresh
                instruction_part15.tStopRefresh = tThisFlipGlobal  # on global time
                instruction_part15.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'instruction_part15.stopped')
                # update status
                instruction_part15.status = FINISHED
                instruction_part15.setAutoDraw(False)
        
        # *continue_button_21* updates
        waitOnFlip = False
        
        # if continue_button_21 is starting this frame...
        if continue_button_21.status == NOT_STARTED and tThisFlip >= 3-frameTolerance:
            # keep track of start time/frame for later
            continue_button_21.frameNStart = frameN  # exact frame index
            continue_button_21.tStart = t  # local t and not account for scr refresh
            continue_button_21.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(continue_button_21, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'continue_button_21.started')
            # update status
            continue_button_21.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(continue_button_21.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(continue_button_21.clearEvents, eventType='keyboard')  # clear events on next screen flip
        
        # if continue_button_21 is stopping this frame...
        if continue_button_21.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > continue_button_21.tStartRefresh + max_read_dur-frameTolerance:
                # keep track of stop time/frame for later
                continue_button_21.tStop = t  # not accounting for scr refresh
                continue_button_21.tStopRefresh = tThisFlipGlobal  # on global time
                continue_button_21.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'continue_button_21.stopped')
                # update status
                continue_button_21.status = FINISHED
                continue_button_21.status = FINISHED
        if continue_button_21.status == STARTED and not waitOnFlip:
            theseKeys = continue_button_21.getKeys(keyList=[left_key, right_key, center_key, down_key], ignoreKeys=["escape"], waitRelease=False)
            _continue_button_21_allKeys.extend(theseKeys)
            if len(_continue_button_21_allKeys):
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                win=win, 
                timers=[routineTimer, globalClock], 
                currentRoutine=instruction_refl_period_02,
            )
            # skip the frame we paused on
            continue
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            instruction_refl_period_02.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in instruction_refl_period_02.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in instruction_refl_period_02.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for instruction_refl_period_02
    instruction_refl_period_02.tStop = globalClock.getTime(format='float')
    instruction_refl_period_02.tStopRefresh = tThisFlipGlobal
    thisExp.addData('instruction_refl_period_02.stopped', instruction_refl_period_02.tStop)
    # check responses
    if continue_button_21.keys in ['', [], None]:  # No response was made
        continue_button_21.keys = None
    thisExp.addData('continue_button_21.keys',continue_button_21.keys)
    if continue_button_21.keys != None:  # we had a response
        thisExp.addData('continue_button_21.rt', continue_button_21.rt)
        thisExp.addData('continue_button_21.duration', continue_button_21.duration)
    thisExp.nextEntry()
    ...
    if language == "german": 
        instruction_info.text = (
    "In den Übungsdurchgängen erhalten Sie unten\n"
    ...
    for thisRetrieval_prc_loop in retrieval_prc_loop:
        ...
        for thisRetry_loop in retry_loop:
            ...
            retr_response_order_prc = data.Routine(
                ...
                components=[yes_txt_3, no_txt_3, polygon_9, resp_3, info_example_7, text_question_order],
            )
            ...
            # Run 'Begin Routine' code from set_text_question_order
            if language=="english":
                text_question_order.text = ("Presented order is order from sequence?")
            if language=="german":
                text_question_order.text = ("Präsentationsreihenfolge entspricht Sequenzreihenfolge?")
            ...
            while continueRoutine:
                ...
                # *text_question_order* updates
                
                # if text_question_order is starting this frame...
                if text_question_order.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    text_question_order.frameNStart = frameN  # exact frame index
                    text_question_order.tStart = t  # local t and not account for scr refresh
                    text_question_order.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(text_question_order, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'text_question_order.started')
                    # update status
                    text_question_order.status = STARTED
                    text_question_order.setAutoDraw(True)
                
                # if text_question_order is active this frame...
                if text_question_order.status == STARTED:
                    # update params
                    pass
                
                # if text_question_order is stopping this frame...
                if text_question_order.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > text_question_order.tStartRefresh + max_read_dur-frameTolerance:
                        # keep track of stop time/frame for later
                        text_question_order.tStop = t  # not accounting for scr refresh
                        text_question_order.tStopRefresh = tThisFlipGlobal  # on global time
                        text_question_order.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'text_question_order.stopped')
                        # update status
                        text_question_order.status = FINISHED
                        text_question_order.setAutoDraw(False)
            ...
            retr_response_distance_prc = data.Routine(
                ...
                components=[resp_2, info_example, opt_2_prc, opt_3_prc, opt_4_prc, opt_5_prc, text_question_retrieval],
            )
            ...
            # Run 'Begin Routine' code from set_text_question_retrieval
            if language=="english":
                text_question_retrieval.text = ("What is the distance between the images in the sequence")
            if language=="german":
                text_question_retrieval.text = ("Wie viele Schritte zum ersten bis zum zweiten Bild?")
            ...
            while continueRoutine:
                ...
                # *text_question_retrieval* updates
                
                # if text_question_retrieval is starting this frame...
                if text_question_retrieval.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    text_question_retrieval.frameNStart = frameN  # exact frame index
                    text_question_retrieval.tStart = t  # local t and not account for scr refresh
                    text_question_retrieval.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(text_question_retrieval, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'text_question_retrieval.started')
                    # update status
                    text_question_retrieval.status = STARTED
                    text_question_retrieval.setAutoDraw(True)
                
                # if text_question_retrieval is active this frame...
                if text_question_retrieval.status == STARTED:
                    # update params
                    pass
                
                # if text_question_retrieval is stopping this frame...
                if text_question_retrieval.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > text_question_retrieval.tStartRefresh + max_read_dur-frameTolerance:
                        # keep track of stop time/frame for later
                        text_question_retrieval.tStop = t  # not accounting for scr refresh
                        text_question_retrieval.tStopRefresh = tThisFlipGlobal  # on global time
                        text_question_retrieval.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'text_question_retrieval.stopped')
                        # update status
                        text_question_retrieval.status = FINISHED
                        text_question_retrieval.setAutoDraw(False)
            ...
            retr_response_feedback = data.Routine(
                ...
                components=[text_feedback, info_example_2, continue_button_16, text_continue],
            )
            ...
            if language == "english" and resp_2.corr==1:
                text_feedback.text = (f"Good job! You understood the retrieval task.")
                
            if language == "german" and resp_2.corr==1:
                text_feedback.text = (f"Gut gemacht! Sie haben Abruf verstanden.")
            ...
            while continueRoutine:
                ...
                if text_feedback.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > text_feedback.tStartRefresh + max_read_dur-frameTolerance:
                        # keep track of stop time/frame for later
                ...
                # if text_continue is stopping this frame...
                if text_continue.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > text_continue.tStartRefresh + max_read_dur-frameTolerance:
                        # keep track of stop time/frame for later
                        text_continue.tStop = t  # not accounting for scr refresh
                        text_continue.tStopRefresh = tThisFlipGlobal  # on global time
                        text_continue.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'text_continue.stopped')
                        # update status
                        text_continue.status = FINISHED
    ...
    for thisBlock in Block:
        ...
        if Block.thisN == 0:
            context_img_block = r"stimuli\context_images\sea_01.jpg"
        if Block.thisN == 1:
            context_img_block = r"stimuli\context_images\forest_01.jpg"
        if Block.thisN == 2:
            context_img_block = r"stimuli\context_images\snow_01.jpg"
        if Block.thisN == 3:
            context_img_block = r"stimuli\context_images\desert_02.jpg"
        ...
        for thisMiniblock in miniblocks:
            ...
            # create an object to store info about Routine show_context
            show_context = data.Routine(
                name='show_context',
                components=[scene_image],
            )
            show_context.status = NOT_STARTED
            continueRoutine = True
            # update component parameters for each repeat
            scene_image.setImage(context_img_block)
            # store start times for show_context
            show_context.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
            show_context.tStart = globalClock.getTime(format='float')
            show_context.status = STARTED
            thisExp.addData('show_context.started', show_context.tStart)
            show_context.maxDuration = None
            # keep track of which components have finished
            show_contextComponents = show_context.components
            for thisComponent in show_context.components:
                thisComponent.tStart = None
                thisComponent.tStop = None
                thisComponent.tStartRefresh = None
                thisComponent.tStopRefresh = None
            ...
            # --- Run Routine "show_context" ---
            show_context.forceEnded = routineForceEnded = not continueRoutine
            while continueRoutine and routineTimer.getTime() < 0.5:
                ...
                # if scene_image is starting this frame...
                if scene_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    ...
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'scene_image.started')
                    # update status
                    scene_image.status = STARTED
                    scene_image.setAutoDraw(True)
                
                # if scene_image is active this frame...
                if scene_image.status == STARTED:
                    # update params
                    pass
                
                # if scene_image is stopping this frame...
                if scene_image.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > scene_image.tStartRefresh + 0.5-frameTolerance:
                        # keep track of stop time/frame for later
                        scene_image.tStop = t  # not accounting for scr refresh
                        scene_image.tStopRefresh = tThisFlipGlobal  # on global time
                        scene_image.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'scene_image.stopped')
                        # update status
                        scene_image.status = FINISHED
                ...
                # check if all components have finished
                if not continueRoutine:  # a component has requested a forced-end of Routine
                    show_context.forceEnded = routineForceEnded = True
                    break
                continueRoutine = False  # will revert to True if at least one component still running
                for thisComponent in show_context.components:
                    if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
            ...
            for thisComponent in show_context.components:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            # store stop times for show_context
            show_context.tStop = globalClock.getTime(format='float')
            show_context.tStopRefresh = tThisFlipGlobal
            thisExp.addData('show_context.stopped', show_context.tStop)
            # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
            if show_context.maxDurationReached:
                routineTimer.addTime(-show_context.maxDuration)
            elif show_context.forceEnded:
                routineTimer.reset()
            else:
            ...
            for thisLearning_trial in learning_trials:
                ...
                choice_display = data.Routine(
                    name='choice_display',
                    components=[scene_image_2, polygon_4, prompt, dist_01, dist_02, correct, chooseNowText, key_resp],
                )
                ...
                while continueRoutine:
                    ...
                    # if scene_image_2 is starting this frame...
                    if scene_image_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                        # keep track of start time/frame for later
                        scene_image_2.frameNStart = frameN  # exact frame index
                        scene_image_2.tStart = t  # local t and not account for scr refresh
                        scene_image_2.tStartRefresh = tThisFlipGlobal  # on global time
                        win.timeOnFlip(scene_image_2, 'tStartRefresh')  # time at next scr refresh
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'scene_image_2.started')
                        # update status
                        scene_image_2.status = STARTED
                        scene_image_2.setAutoDraw(True)
                    
                    # if scene_image_2 is active this frame...
                    if scene_image_2.status == STARTED:
                        # update params
                        pass
                    ...
                    if not responded:
                        ...
                        if len(key_list) > 0:
                            ...
                            # recover the flip-locked onset of the keyboard component
                            t_kb_start = key_resp.tStartRefresh
                            if t_kb_start is None:
                                t_kb_start = key_resp.clock.getLastResetTime()
                                print(
                                    f"[bids] WARNING: key_resp.tStartRefresh was None "
                                    f"falling back to clock reset time "
                                    f"({t_kb_start:.6f}). status={key_resp.status}, "
                                    f"n_keys={len(_key_resp_allKeys)}, frameN={frameN}"
                                )
                    
                            press_onset_est = (t_kb_start + thisResp.rt) - bids.t0
                            
                            bids.add_instant("choice", trial_type="learn",
                            block_name=block, sequence_name=learningSeq,
                            route_num=runIndexWithinSeq, trial_num=currPosInSeq,
                            response=thisResp.name,
                            response_time=thisResp.rt,
                            press_onset_est=press_onset_est,
                            correct=key_resp.corr,
                            expected_response=correct_ans,
                            response_meaning=KeyMeaning)
                    ...
                    if not responded and routClock.getTime() >= (max_response_time-1):
                        chooseNowText.setOpacity(1)
                    ...
                    if key_resp.keys and not resp_trig_giv:
                        bids.trigger(response_trigger, label="image_selected", on_flip=False,
                                     block_name=block, sequence_name=learningSeq,
                                     route_num=runIndexWithinSeq, trial_num=currPosInSeq)
                        trigOn = True
                        resp_trig_giv = True
                ...
                feedback = data.Routine(
                    name='feedback',
                    components=[scene_image_3, polygon_8, prompt_2, dist_01_2, dist_02_2, correct_2],
                )
                ...
                while continueRoutine:
                    ...
                    # if scene_image_3 is starting this frame...
                    if scene_image_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                        # keep track of start time/frame for later
                        scene_image_3.frameNStart = frameN  # exact frame index
                        scene_image_3.tStart = t  # local t and not account for scr refresh
                        scene_image_3.tStartRefresh = tThisFlipGlobal  # on global time
                        win.timeOnFlip(scene_image_3, 'tStartRefresh')  # time at next scr refresh
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'scene_image_3.started')
                        # update status
                        scene_image_3.status = STARTED
                        scene_image_3.setAutoDraw(True)
                    
                    # if scene_image_3 is active this frame...
                    if scene_image_3.status == STARTED:
                        # update params
                        pass
                ...
                too_slow_routine = data.Routine(
                    ...
                    components=[scene_image_4, tooSlowtext],
                )
                ...
                while continueRoutine:
                    ...
                    # if scene_image_4 is starting this frame...
                    if scene_image_4.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                        # keep track of start time/frame for later
                        scene_image_4.frameNStart = frameN  # exact frame index
                        scene_image_4.tStart = t  # local t and not account for scr refresh
                        scene_image_4.tStartRefresh = tThisFlipGlobal  # on global time
                        win.timeOnFlip(scene_image_4, 'tStartRefresh')  # time at next scr refresh
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'scene_image_4.started')
                        # update status
                        scene_image_4.status = STARTED
                        scene_image_4.setAutoDraw(True)
                    
                    # if scene_image_4 is active this frame...
                    if scene_image_4.status == STARTED:
                        # update params
                        pass
                    
                    # if scene_image_4 is stopping this frame...
                    if scene_image_4.status == STARTED:
                        # is it time to stop? (based on global clock, using actual start)
                        if tThisFlipGlobal > scene_image_4.tStartRefresh + too_slow_dur-frameTolerance:
                            # keep track of stop time/frame for later
                            scene_image_4.tStop = t  # not accounting for scr refresh
                            scene_image_4.tStopRefresh = tThisFlipGlobal  # on global time
                            scene_image_4.frameNStop = frameN  # exact frame index
                            # add timestamp to datafile
                            thisExp.timestampOnFlip(win, 'scene_image_4.stopped')
                            # update status
                            scene_image_4.status = FINISHED
            ...
            for thisRetrieval_trial in retrieval_trials:
                ...
                delayClock = None
                delayDone = False
                order_resp_meaning = None
                order_press_onset = None
                ...
                while continueRoutine:
                    ...
                    if not responded_retr:
                        ...
                        if len(key_list) > 0:
                            ...
                            if thisResp.name == left_key:
                                chosenPos = [-0.05, 0]
                                order_resp_meaning = opt_left
                            elif thisResp.name == right_key:
                                chosenPos = [0.05, 0]
                                order_resp_meaning = opt_right
                            resp.keys = thisResp.name
                            resp.rt = thisResp.rt
                            resp.duration = thisResp.duration
                            resp.corr = 1 if thisResp.name == correct_key else 0
                    
                            # flip-locked onset of the response keyboard
                            t_kb_start = resp.tStartRefresh
                            if t_kb_start is None:
                                t_kb_start = resp.clock.getLastResetTime()
                                print(f"[bids] WARNING: resp.tStartRefresh was None (retr_order), "
                                      f"using clock reset {t_kb_start:.6f}, frameN={frameN}")
                            order_press_onset = t_kb_start + thisResp.rt
                    ...
                    if not retr_trig_sent and resp.keys:
                        retr_trig_sent = True
                        bids.trigger(RespTrig, label="order_response", on_flip=False,
                                     block_name=block, sequence_name=learningSeq,
                                     trial_num=retrieval_trials.thisN)
                ...
                bids.add_instant(
                "choice",
                onset=order_press_onset,
                distance_correct = distance_correct,
                trial_type = "retr_order",
                block_name=block,
                sequence_name=learningSeq,
                trial_num=retrieval_trials.thisN,
                response=(resp.keys if responded_retr else "m"),
                response_time=(resp.rt if responded_retr else None),
                correct=is_correct,
                expected_response = correct_key, 
                response_meaning=order_resp_meaning,
                ...
                while continueRoutine:
                    ...
                    # only poll once the component has started AND its flip callbacks have run
                    if resp_4.status == STARTED and resp_4.tStartRefresh is not None:
                        keysPressed = resp_4.getKeys(keyList=[left_key, right_key, center_key, down_key], waitRelease=False)
                    else:
                        keysPressed = []
                    
                    if len(keysPressed) > 0 and not responded_retr_slider:
                        chosen_key = keysPressed[0]
                        if chosen_key.name == left_key:
                            chosenDis = 5
                            opt_5.color = "blue"
                        elif chosen_key.name == right_key:
                        ...
                        resp_4.corr = 1 if chosenDis == distance_correct else 0
                    
                        t_kb_start = resp_4.tStartRefresh
                        if t_kb_start is None:
                            t_kb_start = resp_4.clock.getLastResetTime()
                            print(f"[bids] WARNING: resp_4.tStartRefresh was None (retr_distance), "
                                  f"using clock reset {t_kb_start:.6f}, frameN={frameN}")
                        dist_press_onset = t_kb_start + chosen_key.rt
                    ...
                    if retrClock.getTime() >= retr_dur_slider and not responded_retr_slider:
                        continueRoutine = False
                    ...
                    if not retr_trig_sent and resp_4.keys:
                        retr_trig_sent = True
                        bids.trigger(RespTrig, label="distance_response", on_flip=False,
                                     block_name=block, sequence_name=learningSeq,
                                     trial_num=retrieval_trials.thisN)
                        core.wait(0.01) # need to wait a bit to avoid overlap with question trigger
                ...
                bids.add_instant(
                "choice",
                onset=dist_press_onset,
                distance_correct=distance_correct,
                trial_type="retr_distance",
                block_name=block,
                sequence_name=learningSeq,
                trial_num=retrieval_trials.thisN,
                response=(chosenDis if responded_retr_slider else "m"),
                response_time=(resp_4.rt if responded_retr_slider else None),
                correct=is_correct,
